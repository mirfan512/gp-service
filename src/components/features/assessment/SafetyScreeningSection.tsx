...
interface SafetyScreeningSectionProps {
  values: YesNo[];
  onChange: (index: number, v: YesNo) => void;
}

export function SafetyScreeningSection({ values, onChange }: SafetyScreeningSectionProps) {
  return (
    <>
      <FormSectionHeader>Contraindications & Safety Screening</FormSectionHeader>
      <div className="mt-6 grid gap-5">
        {ASSESSMENT_QUESTIONS.CONTRAINDICATIONS.map((q, idx) => (
